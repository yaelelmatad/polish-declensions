...
32. **Verbal noun practice page** — `/verbal-nouns` route: practice forming verbal nouns (rzeczowniki odczasownikowe) from verbs and converting accusative objects to genitive. Data in `src/data/verbalNounData.ts` (62 verbs with formation rules + 89 sentences). Route: `src/routes/VerbalNounPractice.tsx`. **Two separate input fields** labeled "verb" and "noun" (noun only appears when sentence has an object). Formation rule and verbal noun form are NOT shown during the question — only in hints and feedback. Collapsible sticky "Formation rules" reference at top of page. Three types of questions: (a) verbal noun only ("biegać → bieganie"), (b) verbal noun + object case change ("czytać książkę → czytanie książki"), (c) "do + genitive" fixed expressions ("maszyna do pisania", "coś do jedzenia"). Formation rules: -ać → -anie, -ować → -owanie, -ić/-yć → -enie, -eć → -enie, short verbs → -cie, -ąć → -ęcie (irregular). Key grammar: acc objects become genitive with verbal nouns. **Design principle:** this page focuses on *forming* verbal nouns and the object case change, NOT on declining the verbal noun itself through cases (that belongs in the Declension tab). Avoid "nie lubię" / "unikam" constructions that force the verbal noun into genitive — keep the verbal noun in nominative (except "do + gen" fixed expressions which are a distinct pattern). Shows "your answer" when incorrect; diacritics hints shown regardless of correctness. Includes SRS (`srs-verbal-noun` deck), report mistake, progress tracking.
...
- **Verbal nouns:** Verbs in `verbalNounData.ts` have `verbalNoun` and `rule` fields. Sentences use `{v}` and `{n}` template placeholders and have `expectedVerb` + optional `expectedNoun` (split into two fields, not a combined string). `verbAlternates` / `nounAlternates` for accepted variants. `originalPhrase` shows what the user converts from. Only use verbIds that exist in `verbalNounData.ts`. Covers regular patterns (-ać→-anie, -ować→-owanie, -ić/-yć→-enie, -eć→-enie, short→-cie), irregular -ąć→-ęcie (wziąć→wzięcie, zdjąć→zdjęcie, etc.), consonant alternations (nosić→noszenie, wozić→wożenie), and irregular stems (jeść→jedzenie). Many verbal nouns double as everyday words (mieszkanie=apartment, spotkanie=meeting, zdjęcie=photo, przyjęcie=party, pytanie=question, etc.). Do NOT add sentences where the verbal noun itself must be declined (e.g. "nie lubię gotowania") — keep the verbal noun in nominative except for "do + gen" fixed expressions.
